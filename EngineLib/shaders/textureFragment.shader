...
// Final colour is output for per-sample operations step
out vec4 colour;
...
vec4 calculateDiffuse() {
	...
	// Final diffuse value is a factor of its colour, dot product modifier and intensity
	return vec4(light.colour, 1.0) * diffuseMultiplier * light.dIntensity;
}

vec4 calculateSpecular() {
	// Check that there is diffuse light on this fragment
	// No diffuse -> no specular
	if(diffuseMultiplier > 0) {
		...
		if(specularMultiplier > 0) {
			// The shininess of an object affects how much specular light there will be
...
void main() {
	...
	colour = texture(tex, textureCoord) * (ambient + diffuse + specular);
}